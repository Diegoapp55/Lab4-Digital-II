...
static void display_test(void)
{
	...
	printf("Test de los display de 7 segmentos... se interrumpe con el botton 1\n");
...
static void led_test(void) //Revisar por que los últimos leds de la Nexys no funcionan
{
	unsigned int i;
	printf("Test de los leds... se interrumpe con el botton 1\n");
	while(!(buttons_in_read()&1)) {
...
static void switch_test(void)
{
	...
	printf("Test de los interruptores... se interrumpe con el botton 1\n");
	while(!(buttons_in_read()&1)) {
...
{
	unsigned int T = 128;
	printf("Test del led rgb... se interrumpe con el botton 1\n");
	while(!(buttons_in_read()&1)) {
	...
	while(!(buttons_in_read()&1)) {
		...
		if (temp2 != temp){
			...
			if(temp == 1){
				...
				// Contador de 65535 hasta 1
				for(i=65535; i>1; i=i-1) {
					leds_out_write(i);
					delay_ms(500);
					temp = switchs_in_read();
					if(temp == 0){i=1;}
